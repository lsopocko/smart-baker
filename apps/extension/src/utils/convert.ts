...
const VOLUME_CONVERSIONS_TO_ML: Record<VolumeUnit, number> = {
  cup: 240,
  cups: 240,
  teaspoons: 4.93,
  teaspoon: 4.93,
...
const WEIGHT_CONVERSIONS_TO_G: Record<WeightUnit, number> = {
  oz: 28.35,
  lb: 453.6
};
...
export function convertToMetric(
  ...
  if (unit in VOLUME_CONVERSIONS_TO_ML) {
    const ml = amount * VOLUME_CONVERSIONS_TO_ML[unit as VolumeUnit];
